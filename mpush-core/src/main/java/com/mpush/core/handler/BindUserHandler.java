...
public final class BindUserHandler extends BaseMessageHandler<BindUserMessage> {
    ...
    private void bind(BindUserMessage message) {
        if (Strings.isNullOrEmpty(message.userId)) {
            ...
            Logs.CONN.error("bind user failure for invalid param, connection={}", message.getConnection());
            return;
        }
        //1.绑定用户时先看下是否握手成功
        ...
        if (context.handshakeOk()) {
            ...
            Logs.CONN.error("bind user failure not handshake, userId={}, connection={}", message.userId, message.getConnection());
        }
    }
