...
public final class HandshakeHandler extends BaseMessageHandler<HandshakeMessage> {
    ...
    public void handle(HandshakeMessage message) {
        ...
        if (Strings.isNullOrEmpty(message.deviceId)
            ...
            Logs.CONN.error("handshake failure, message={}, connection={}", message, message.getConnection());
            return;
        }
        ...
        if (message.deviceId.equals(context.deviceId)) {
            Logs.CONN.warn("handshake failure, repeat handshake, session={}", message.getConnection().getSessionContext());
            return;
        }
        ...
        Logs.CONN.info(">>> handshake success, connection={}", message.getConnection());
    }
}