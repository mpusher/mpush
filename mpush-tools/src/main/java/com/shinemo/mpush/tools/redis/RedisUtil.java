...
public class RedisUtil {
	...
	public static <T> T get(RedisNode node, String key, Class<T> clazz) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis get exception:%s,%s",key,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static void set(List<RedisNode> nodeList, String key, String value, Integer time) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis set exception:%s,%s,%s,%s",key,value,time,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static void del(List<RedisNode> nodeList, String key) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis del exception:%s,%s",key,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static void hset(List<RedisNode> nodeList, String namespace, String key, String value) {
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis hset exception:%s,%s,%s,%s",namespace,key,value,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static <T> T hget(RedisNode node, String namespace, String key, Class<T> clazz) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis hget exception:%s,%s",namespace,key,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static void hdel(List<RedisNode> nodeList, String namespace, String key) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis hdel exception:%s,%s,%s",namespace,key,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static Map<String, String> hgetAll(RedisNode node, String namespace) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis hgetAll exception:%s,%s",namespace,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static Set<String> hkeys(RedisNode node, String key) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis hkeys exception:%s,%s",key,node);
        } finally {
            // 返还到连接池
            close(jedis);
    ...
    public static <T> List<T> hmget(RedisNode node, String namespace, Class<T> clazz, String... key) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis hmget exception:%s,%s,%s",namespace,key,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static void hmset(List<RedisNode> nodeList, String namespace, Map<String, String> hash, Integer time) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis hmset exception:%s,%s,%s",namespace,time,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static void lpush(List<RedisNode> nodeList, String key, String value) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis lpush exception:%s,%s,%s",key,value,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static void rpush(List<RedisNode> nodeList, String key, String value) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis rpush exception:%s,%s,%s",key,value,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static <T> T lpop(List<RedisNode> nodeList, String key, Class<T> clazz) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis lpop exception:%s,%s",key,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static <T> T rpop(List<RedisNode> nodeList, String key, Class<T> clazz) {
        ...
        for (RedisNode node : nodeList) {
            ...
            try {
            	...
            	LoggerManage.log(LogType.REDIS, e, "redis rpop exception:%s,%s",key,node);
            } finally {
                // 返还到连接池
                close(jedis);
            }
    ...
    public static <T> List<T> lrange(RedisNode node, String key, int start, int end, Class<T> clazz) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis lrange exception:%s,%s,%s,%s",key,start,end,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static long llen(RedisNode node, String key) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis llen exception:%s,%s",key,node);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static <T> void publish(RedisNode node, String channel, T message) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis publish exception:%s,%s,%s",value,node,channel);
        } finally {
            // 返还到连接池
            close(jedis);
        }
    ...
    public static void subscribe(RedisNode node, JedisPubSub pubsub, String... channel) {
        ...
        try {
        	...
        	LoggerManage.log(LogType.REDIS, e, "redis subscribe exception:%s,%s",node,channel);
        } finally {
            // 返还到连接池
            close(jedis);
        }
